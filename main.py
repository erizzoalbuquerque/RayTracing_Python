...
from source.RayTracing.shapes import Sphere, Plane, Box
...
def CreateObjects():
    ...
    white_floor = ObjectInstance(
        Transform(glm.vec3(0, 0, 0)),
        Plane(glm.vec3(0, 1, 0)),
        PhongMaterial(Color(1, 1, 1), Color(0, 0, 0), 10)
    )
    ...
    instances.append(
        ObjectInstance(
            Transform(
                glm.vec3(0.7, 1.25, 1.1),
                glm.vec3(0, 45, 0)
            ),
            Box(glm.vec3(1, 2.5, 1)),
            PhongMaterial(Color(1, 1, 1), Color(1, 1, 1), 10)
        )
    )
...
def CreateLights():
    ...
    area_light = LightInstance(
        Transform(glm.vec3(-1, 4, 0)),
        Box(glm.vec3(2.1, 0.1, 1.1)),
        AreaLight(15, glm.vec3(2, 0, 0), glm.vec3(0, 0, 1), 4, "STRATIFIED")
    )
